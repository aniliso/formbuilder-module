...
class FormController extends BasePublicController
{
    ...
    public function send(FormcustomRequest $request)
    {
        ...
        if (!$this->checkCaptcha($fields, $image, $data)) {
            return redirect()->back()->withInput()->withError(trans('formbuilder::formbuilder.message.error_captcha'));
        }

        unset($data['captchainput']);
     ...
     * @return array
     */
    private function handleFiles($files, $formId, $data)
    {
        $dataFiles = array();
        foreach ($files as $key => $file) {
            if ($file->isValid()) {
     ...
     * @return bool
     */
    private function checkCaptcha($fields, $image, $data)
    {
